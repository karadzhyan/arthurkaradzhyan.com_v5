...
export var crossIndustryPatterns = [
  {
    ...
    summary: "Three industries face the same structural problem: their operational models make full meal period compliance genuinely difficult, and the Donohue presumption converts the resulting time-clock patterns into rebuttable violations at scale. The defense framework — temporal bifurcation — works across all three, adapted to each sector's evidence structure.",
    description: "The core vulnerability is identical across hospitality, healthcare staffing, and agriculture: the employer's operational model conflicts with California's rigid meal period requirements, generating time-clock data that triggers the Donohue presumption even when breaks were offered. In hospitality, 24/7 guest-facing operations mean that 'relieved of all duties' is in tension with the service model. In healthcare staffing, CDPH staffing ratios at client facilities create scheduling constraints the staffing firm cannot control. In agriculture, remote field locations and piece-rate production pressure make compliant break provision logistically difficult. The common defense opportunity: temporal bifurcation works across all three sectors, using different evidence in each — attestation systems in hospitality, facility-specific control matrices in healthcare, field-level documentation in agriculture.",
  ...
  },
  {
    ...
    summary: "Every piece-rate employer carries an embedded underpayment in every pay period for every affected worker — because most legacy payroll systems cannot perform the separate rest period and non-productive time calculations that section 226.2 requires. The exposure compounds across years and generates derivative wage statement violations.",
    description: "The compliance failure is the same in solar and agriculture: section 226.2 requires separate compensation for rest periods at a regular rate derived from piece-rate earnings, and separate compensation for non-productive time at not less than minimum wage, with each component separately identified on the wage statement. Most legacy payroll systems predate this requirement and cannot perform these calculations. Solar installers paid per panel and agricultural workers paid per bin share the identical exposure — systematic underpayment compounding across every pay period, with derivative section 226(a) wage statement violations for every affected worker. The payroll audit methodology is identical across sectors; only the operational context differs.",
  ...
  },
  {
    ...
    summary: "No published authority addresses how PAGA penalties should be allocated between joint employers — creating a defense opportunity for every employer bearing liability for violations it could not control. The control-matrix framework developed for healthcare staffing applies directly to agricultural grower/contractor arrangements.",
    description: "The defense opportunity is the same in both sectors: the employer of record bears PAGA liability for violations occurring in workplaces it does not control. A staffing firm is liable when a county hospital's charge nurse prevents a meal break at minimum staffing. A grower is liable when a labor contractor's payroll system fails to calculate piece-rate rest period compensation. Martinez v. Combs establishes joint employer status in both cases, but no published decision addresses penalty allocation based on which entity controlled the violation-generating conditions. The control-matrix framework — documenting which entity controls scheduling, break timing, overtime authorization, and payroll for each violation category — works identically in both sectors.",
  ...
  },
  {
    ...
    summary: "Most payroll systems understate the regular rate because they fail to include all compensation components — commissions, bonuses, spiffs, equity vesting. The per-hour underpayment is small, but it multiplies across every overtime and premium hour for every affected employee, compounding into substantial aggregate exposure.",
    description: "The regular rate problem is identical in structure across three industries, differing only in which compensation components the payroll system mishandles. Dealerships layer base salary, commissions, manufacturer spiffs, holdback bonuses, CSI bonuses, and volume escalators. Technology companies add RSU vesting, equity grants, and performance bonuses. Hospitality employers add tipped and service-charge-based compensation. In all three, Alvarado v. Dart Container requires flat-sum bonuses to be divided by total hours worked in the bonus period, and Ferra v. Loews Hollywood Hotel requires meal and rest period premiums at the regular rate, not the base hourly rate. Most payroll systems fail both calculations. The Regular Rate Calculator models the correct rate and quantifies the delta.",
  ...
  },
  {
    ...
    summary: "Unpaid travel time from employer-designated reporting locations to worksites generates daily overtime exposure before substantive work even begins. In both solar and healthcare, the travel adds 1-2 hours per day that push total hours past the overtime threshold — compounding across every workday for every affected employee.",
    description: "The exposure mechanism is the same in both industries: the employer requires reporting to a specific location (a yard, a facility), and the subsequent travel to the actual worksite is compensable under Morillion. Solar field crews drive from an employer-designated yard to remote installation sites. Healthcare workers travel between client facilities during the same day. In both cases, the unpaid travel time pushes total daily hours beyond 8, generating overtime exposure before the substantive work begins. The defense turns on the same factual distinction in both sectors: was reporting to the specific location employer-required (compensable) or voluntary (non-compensable commute)?",
  ...
  },
  {
    ...
    summary: "Multi-location employers can limit or restructure PAGA claims under section 2699(p) by demonstrating that each worksite presents a distinct compliance environment — making representative treatment analytically inadequate. This defense is available whenever workplace conditions vary materially across locations.",
    description: "The defense opportunity is structural: when each location has different scheduling software, break policies, supervisory structures, and operational constraints, a unified PAGA claim treats fundamentally different workplaces as interchangeable. Healthcare staffing firms with 14 client facilities, hospitality operators with 12 hotel properties, and agricultural operations spanning multiple fields with different labor contractors all satisfy this threshold. The Estrada v. Royalty Carpet Mills framework supports arguing that representative treatment cannot accommodate the individualized proof requirements these operations demand. The evidence is location-specific in all three sectors — different compliance systems, different supervisory practices, different workforce demographics — making the manageability motion structurally identical across industries.",
...
var industriesRaw = [
  {
    ...
    exposureCategories: [
      ...
      },
      {
        ...
        analysis: "Five to ten minutes of unpaid work per shift, multiplied across hundreds of employees and every pay period in the PAGA statutory period, creates substantial aggregate exposure — and after Troester v. Starbucks eliminated the federal de minimis doctrine for California wage claims, pre-shift setup tasks (logging into POS systems, counting cash drawers, pre-shift meetings) and post-shift closing tasks (cleaning, restocking, end-of-day reports) are all compensable. In hospitality operations with hundreds of employees each performing these tasks daily, the aggregate exposure is substantial — and it compounds across every pay period in the PAGA statutory period.",
      ...
      },
      {
        ...
        analysis: "Most hotels do not recognize that per-room housekeeping pay constitutes piece-rate compensation under section 226.2 — creating an exposure category that virtually no hospitality employer has addressed. Room attendants compensated on a per-room or per-floor basis are piece-rate employees, meaning the hotel must separately calculate and pay rest period time at the piece-rate regular rate (not minimum wage). Non-productive time (waiting for room assignments, attending meetings, traveling between floors) must also be separately compensated at no less than the applicable minimum wage. Most hotel payroll systems do not distinguish between productive and non-productive time for housekeeping staff, creating systematic underpayment. The wage statement must separately identify each compensation component — a requirement that virtually no legacy hotel payroll system satisfies.",
      ...
      },
      {
        ...
        analysis: "In hospitality, the line between exempt managerial work and non-exempt production work is blurred — and it routinely fails the 50% threshold. The executive exemption requires that exempt duties (managing the enterprise, directing two or more employees, exercising discretion and independent judgment) occupy more than half of working time. a restaurant manager who regularly works the floor during rush periods, a hotel front desk supervisor who checks in guests during peak times, or a housekeeping manager who cleans rooms when short-staffed is performing non-exempt duties that may push the time allocation below 50%. The salary basis test adds a second vulnerability — managers paid a salary but subject to deductions for partial-day absences or performance-based reductions may lose the exemption entirely.",
    ...
    exposureCategories: [
      {
        ...
        analysis: "Every salesperson who departed between deal closing and deal funding has a claim for unpaid earned wages — because under Sciborski, commissions are earned when the employee completes the work entitling them to the commission, and for car salespeople, that moment is deal closing, not deal funding. The exposure compounds: plus derivative § 203 waiting time penalties (up to 30 days of daily wages per employee) and § 226 wage statement violations (for failing to report the earned but unpaid commission). The exposure compounds: a dealership with 20% annual salesperson turnover and an average of 2 pending deals per departed salesperson creates 8-12 claimants per year, each with commission, waiting time, and wage statement exposure.",
      ...
      },
      {
        ...
        analysis: "Dealerships that verify the commissioned-employee overtime exemption monthly or quarterly — rather than workweek-by-workweek as required — are mispricing the exemption and creating overtime exposure they have not calculated. The Wage Order 7 exemption requires that (1) more than half of compensation is commissions and (2) total earnings exceed 1.5 times minimum wage, verified each workweek. During slow sales months, a commissioned salesperson can fall below the 1.5 times threshold for specific workweeks, creating overtime exposure for those weeks even if the annual average exceeds the threshold. Dealerships that rely on monthly or quarterly true-ups rather than workweek-by-workweek verification are mispricing the exemption.",
      ...
      },
      {
        ...
        analysis: "Most dealership payroll systems understate the regular rate because they fail to include all compensation components — creating systematic underpayment that compounds across every affected pay period. Dealership compensation typically layers base salary or draw, commissions, manufacturer spiffs, holdback bonuses, CSI bonuses, and volume escalators. Under Alvarado, flat-sum bonuses must be divided by total hours worked in the bonus period — not by 40 hours. Under Ferra, meal and rest period premiums must be calculated at the regular rate, not the base hourly rate. Most payroll systems fail both calculations, creating systematic underpayment that compounds across every affected pay period.",
      ...
      },
      {
        ...
        analysis: "Every bonus and incentive component that dealerships layer on top of base compensation must be included in the regular rate calculation — and most payroll systems exclude them all, creating systematic overtime underpayment. The components include manufacturer spiffs (per-unit bonuses for selling specific models), holdback bonuses (deferred compensation released upon deal funding), CSI bonuses (tied to survey scores), and volume escalators (triggered when monthly sales thresholds are met). The calculation methodology varies by bonus type: flat-sum bonuses are divided by total hours worked in the bonus period per Alvarado, while production bonuses tied to specific units may be allocated differently. Most dealership payroll systems treat these bonuses as separate from the regular rate, creating systematic underpayment.",
    ...
    exposureCategories: [
      {
        ...
        analysis: "The allocation of PAGA penalties between joint employers is uncharted — no published decision addresses whether the staffing agency bears the full penalty burden for violations caused by the client facility's operational decisions. Martinez established a three-part employment test: (1) exercise control over wages, hours, or working conditions, (2) suffer or permit to work, or (3) engage. In staffing arrangements, both entities may satisfy one or more prongs, creating joint liability. But the allocation question is unresolved — no published decision addresses whether the staffing agency bears the full penalty burden for violations caused by the client facility's operational decisions.",
      ...
      },
      {
        ...
        analysis: "A structural inequity: the entity that controls the conditions generating the violation (the public hospital) is immune from PAGA penalties, while the entity that cannot control those conditions (the staffing firm) bears the full penalty. When a staffing firm places healthcare workers at government-operated hospitals, Government Code sections 815 through 818 may shield the public agency — but the staffing firm, as employer of record, has no parallel immunity. This creates the inequity: the entity that controls the conditions generating the violation (the public hospital) is immune, while the entity that cannot control those conditions (the staffing firm) bears the penalty. No published appellate decision has addressed whether PAGA penalties should attach to a secondary employer for violations arising from conditions controlled by an immune primary employer.",
      ...
      },
      {
        ...
        analysis: "Healthcare on-call arrangements frequently fall on the compensable side of the line — because the response time requirements are short and the geographic restrictions are significant. Under Mendiola, compensability turns on the degree of employer control during on-call periods. If the staffing agency requires the employee to remain within a geographic radius, respond within a short timeframe, or refrain from personal activities during on-call periods, the time may be compensable as hours worked. The 'engaged to wait' versus 'waiting to be engaged' distinction is fact-intensive — and healthcare on-call arrangements frequently fall on the compensable side because the response time requirements are short and the geographic restrictions are significant.",
      ...
      },
      {
        ...
        analysis: "Many staffing agencies provide per diem stipends that do not satisfy section 2802 — either because the amount is inadequate or because the per diem is structured as taxable compensation rather than a non-taxable reimbursement. Traveling healthcare workers incur substantial expenses (temporary housing, meals, transportation, licensing fees), and section 2802 requires reimbursement of all necessary expenditures incurred in direct consequence of job duties. The distinction between a per diem payment (which may or may not satisfy the reimbursement obligation) and an actual expense reimbursement (which does) creates PAGA exposure when the per diem is inadequate or improperly structured.",
  ...
  },
  {
    ...
    exposureCategories: [
      ...
      },
      {
        name: "Piece-Rate Non-Productive Time",
        statute: "Lab. Code § 226.2",
        analysis: "Most legacy payroll systems cannot perform the separate rest period and non-productive time calculations that section 226.2 requires for piece-rate workers — creating both a wage claim and a derivative wage statement violation for every pay period. Solar installers paid per panel or per system must receive separate compensation for rest periods at the piece-rate regular rate, and separate compensation for non-productive time at not less than minimum wage, with each component identified on the wage statement.",
      ...
      },
      {
        ...
        analysis: "Unpaid travel time from employer-designated yards to remote installation sites generates 1-2 hours of additional compensable time per day — pushing total daily hours well beyond 8 and creating overtime exposure before installation work even begins. Under Morillion, when the employer requires reporting to a specific location, the travel time from that location to the worksite is compensable — it is not a non-compensable commute. This travel time pushes total daily hours well beyond 8 even before installation work begins. On a 4/10 AWS, a crew that spends 1 hour traveling each way works 12 hours (10 + 2 travel), generating 2 hours of unpaid overtime per day if travel time is not compensated. If the AWS is also invalid, the overtime calculation resets to an 8-hour day, generating 4 hours of unpaid overtime per day.",
      ...
      },
      {
        ...
        analysis: "The absence of a comprehensive reimbursement policy creates per-employee, per-pay-period PAGA exposure for every field worker — and most solar employers have gaps. Section 2802 requires reimbursement of all necessary expenditures incurred in direct consequence of job duties: personal vehicle mileage, cell phone use for job-site coordination, personal tools, safety gear replacement, and required work clothing. Many solar employers provide company vehicles and some equipment but do not reimburse for personal cell phone use, tool wear, or other out-of-pocket expenses.",
      ...
      },
      {
        ...
        analysis: "Heat illness prevention violations are PAGA-actionable under section 6720 — and on remote rooftop installation sites, compliance with shade and water requirements presents logistical challenges that many solar employers have not adequately addressed. Cal/OSHA requires shade access above 80 degrees, water provision (one quart per hour per employee), cool-down rest periods of at least five minutes, and training for all employees and supervisors. Solar installation is outdoor work in direct sun exposure, often on rooftops where reflected heat pushes effective temperatures well above ambient. Each violation creates PAGA penalty exposure separate from Cal/OSHA enforcement penalties.",
    ...
    exposureCategories: [
      {
        ...
        analysis: "Four categories of technology workers are routinely misclassified under the computer professional exemption — each generating overtime, meal period, and rest period exposure for every workweek of misclassification. The exemption has both a salary threshold (among the highest of any California exemption, adjusting annually) and a duties test requiring that the employee be 'primarily engaged' in intellectual or creative work with discretion and independent judgment. Roles that commonly fail: QA engineers whose work is primarily testing against predefined specifications, IT support staff who follow established troubleshooting protocols, junior developers who spend substantial time on rote tasks, and project managers whose work is organizational rather than technical. Each misclassified employee creates overtime, meal period, and rest period exposure for every workweek of misclassification.",
      ...
      },
      {
        ...
        statute: "Lab. Code § 2802",
        analysis: "The absence of any reimbursement policy creates per-employee, per-pay-period PAGA exposure — and this is the single most common PAGA-actionable gap in technology companies. Section 2802 requires reimbursement of all necessary expenditures incurred in discharging job duties. For remote workers: a reasonable portion of home internet, cell phone, computer equipment, office furniture, and supplies. The 'reasonable portion' calculation is employer-specific — a $50/month flat stipend may be adequate for some employees and inadequate for others depending on actual costs.",
      ...
      },
      {
        ...
        analysis: "Most payroll systems treat equity compensation as separate from the regular rate — creating systematic overtime underpayment for every employee who receives equity and works overtime hours. When RSUs vest on a schedule, the value at vesting must be included in the regular rate calculation for the vesting period. The magnitude depends on the equity component's proportion of total compensation, which in technology companies can be substantial.",
  ...
  },
  {
    ...
    exposureCategories: [
      ...
      },
      {
        ...
        analysis: "Violations are systematic in agricultural operations — inadequate shade structures in remote fields, insufficient water supply during peak harvest, and supervisor-level pressure to continue working through rest periods — and each violation creates PAGA penalty exposure separate from Cal/OSHA enforcement penalties. Heat illness prevention is PAGA-actionable for outdoor agricultural work. Cal/OSHA requires shade access above 80 degrees, water provision (one quart per hour per employee), cool-down rest periods (at least five minutes), and training for all employees and supervisors.",
      ...
      },
      {
        ...
        analysis: "Field sanitation violations are among the most common Cal/OSHA citations in agriculture — and they are PAGA-actionable under section 6712. In remote agricultural fields, compliance is logistically challenging: portable facilities must be transported, maintained, and repositioned as crews move through different sections of the operation. The regulations require toilet facilities within a quarter-mile walk or five minutes of each employee's work area, hand-washing facilities with soap and single-use towels, drinking water in sufficient quantity, and shaded rest areas above 80 degrees. The logistical challenge is significant — portable facilities must be transported, maintained, and repositioned as crews move through different sections of the operation. Violations are PAGA-actionable under section 6712's private right of action for health and safety violations, and they are among the most common Cal/OSHA citations in agriculture.",
      ...
      },
      {
        ...
        analysis: "Many agricultural operations have not updated their payroll systems to calculate daily overtime — creating systematic underpayment for every affected pay period since the transition. AB 1066 phased out the agricultural overtime exemption, and as of January 1, 2022, agricultural workers at employers with 26+ employees are entitled to overtime after 8 hours in a day and 40 hours in a week (the same standard as all other California workers). Harvest schedules, weather-dependent work patterns, and production quotas frequently result in workdays exceeding 8 hours — and if the payroll system has not been updated to calculate daily overtime on the non-agricultural standard, every affected pay period generates overtime underpayment exposure. The transition from weekly-only overtime to daily overtime represents a fundamental change in how agricultural payroll must be calculated.",
