  ...
  return (
    <div style={{ fontFamily: "'Libre Baskerville',Georgia,serif", color: '#1a1a1a', background: '#fff', minHeight: '100vh' }}>
      <nav style={{ padding: '22px 48px', borderBottom: '1px solid #eee', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <Link href="/" style={{ fontFamily: "'Outfit',sans-serif", fontSize: 10, letterSpacing: 3, textTransform: 'uppercase', color: '#999', textDecoration: 'none' }}>
          ← Home
        </Link>
      </nav>
          ...
          return (
            <Link key={item.slug} href={'/commentary/' + item.slug} style={{ textDecoration: 'none', display: 'block', padding: '28px 0', borderBottom: '1px solid #eee' }}>
              <div style={{ display: 'flex', gap: 20, alignItems: 'flex-start' }}>
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 18, fontWeight: 700, color: '#1a1a1a', lineHeight: 1.35, marginBottom: 8 }}>
                    {item.title}
                  </div>
                  <div style={{ fontFamily: "'Outfit',sans-serif", fontSize: 13, color: '#888', lineHeight: 1.7 }}>
                    {item.summary}
                  </div>
                  <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginTop: 10 }}>
                    {item.tags && item.tags.map(function(tag, j) {
                      return (
                        <span key={j} style={{ fontFamily: "'Outfit',sans-serif", fontSize: 8, fontWeight: 600, letterSpacing: 1, textTransform: 'uppercase', color: '#2c3e3a', padding: '2px 6px', background: '#f0f5f4' }}>
                          {tag}
                        </span>
                      );
                    })}
